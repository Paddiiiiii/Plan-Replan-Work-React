...
class KnowledgeUnitSchemaFreeExtractor(ExtractorABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.SCHEMA_DICT = {
            ...
            "障碍物": "Obstacle",
            "Obstacle": "Obstacle",
            "防御阵地": "DefensePosition",
            "DefensePosition": "DefensePosition",
            "战斗阵地": "CombatPosition",
            "CombatPosition": "CombatPosition",
            "单位组织": "UnitOrganization",
    ...
    def _named_entity_recognition_process(self, passage, ner_result):
        ...
        for item in extra_ner_result:
            ...
            if spg_type is None:
                ...
                if namespace and not label.startswith(namespace + "."):
                    label_with_namespace = f"{namespace}.{label}"
                    spg_type = self.schema.get(label_with_namespace)
                    if spg_type:
                        label = label_with_namespace
            
            if spg_type is None:
                # 如果找不到对应的schema类型，跳过该实体
                logger.warning(f"Schema type not found for label '{label}', entity '{name}' will be skipped. Available types: {list(self.schema.keys())[:20]}")
                continue
    ...
    def named_entity_recognition(self, passage: str):
        ...
        if not ner_parse_rst:
            raise ValueError(f"Named entity recognition returned empty result for passage: {passage[:100]}...")
    ...
    def assemble_sub_graph_with_spg_properties(
        ...
        if spg_type is None:
            # 尝试添加命名空间前缀
            namespace = self.kag_project_config.namespace
            if namespace and not s_label.startswith(namespace + "."):
                s_label_with_namespace = f"{namespace}.{s_label}"
                spg_type = self.schema.get(s_label_with_namespace)
                if spg_type:
                    s_label = s_label_with_namespace
        
        # 如果找不到对应的schema类型，记录警告并跳过该实体
        if spg_type is None:
            logger.warning(f"Schema type not found for label '{s_label}', entity '{s_name}' will be skipped. Available types: {list(self.schema.keys())[:20]}")  # 只显示前20个类型
            return
    ...
    def assemble_knowledge_unit(
        ...
        for knowledge_name, knowledge_value in knowledge_units.items():
            ...
            for item in knowledge_value.get("core_entities", "").split(","):
                if not item.strip():
                    continue
                # 先尝试从source_entities中查找类型
                ent_type = None
                for source_entity in source_entities:
                    if item.strip() == source_entity.get("name", ""):
                        ent_type = source_entity.get("category", "")
                        break
                # 如果找到了类型，添加到core_entities
                if ent_type:
                    core_entities[item.strip()] = ent_type
                # 如果没找到，跳过该core_entity（不再使用Others作为默认类型）
            ...
            for core_entity, ent_type in core_entities.items():
                ...
                # 验证类型是否在schema中存在
                spg_type = self.schema.get(ent_type)
                if spg_type is None:
                    # 尝试添加命名空间前缀
                    namespace = self.kag_project_config.namespace
                    if namespace and not ent_type.startswith(namespace + "."):
                        ent_type_with_namespace = f"{namespace}.{ent_type}"
                        spg_type = self.schema.get(ent_type_with_namespace)
                        if spg_type:
                            ent_type = ent_type_with_namespace
                # 如果仍然找不到类型，跳过该实体
                if spg_type is None:
                    logger.warning(f"Schema type not found for core_entity '{core_entity}' with type '{ent_type}', will be skipped.")
                    continue
